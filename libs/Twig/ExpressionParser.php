        ...
        while(!$stream->test(Twig_Token::PUNCTUATION_TYPE, '}')) {
            ...
            elseif($stream->test(Twig_Token::PUNCTUATION_TYPE, '(')) {
                ...
                throw new Twig_Error_Syntax(sprintf('A hash key must be a quoted string, a number, a name, or an expression enclosed in parentheses (unexpected token "%s" of value "%s"',
                                                    Twig_Token::typeToEnglish($current->getType(), $current->getLine()),
    ...
    public function getFunctionNode($name, $line) {
        ...
        switch($name) {
            ...
            case 'attribute':
                ...
                return new Twig_Node_Expression_GetAttr($args->getNode(0), $args->getNode(1), count($args) > 2 ?
                    $args->getNode(2) : new Twig_Node_Expression_Array(array(), $line), Twig_TemplateInterface::ANY_CALL, $line);
            default:
    ...
    public function parseSubscriptExpression($node) {
        ...
        if($token->getValue() == '.') {
            ...
            if($stream->test(Twig_Token::PUNCTUATION_TYPE, ':')) {
                ...
                $filter =
                    new $class($node, new Twig_Node_Expression_Constant('slice', $token->getLine()), $arguments, $token->getLine());
