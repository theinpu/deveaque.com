...
class Twig_Parser implements Twig_ParserInterface {
    ...
    public function getVarName() {
        return sprintf('__internal_%s_%d', substr($this->env->getTemplateClass($this->stream->getFilename()),
                                                  strlen($this->env->getTemplateClassPrefix())), ++$this->tmpVarCount);
    }
    ...
    public function parse(Twig_TokenStream $stream) {
        ...
        $node =
            new Twig_Node_Module(new Twig_Node_Body(array($body)), $this->parent, new Twig_Node($this->blocks), new Twig_Node($this->macros), new Twig_Node($this->traits), $this->stream->getFilename());
        ...
        while(!$this->stream->isEOF()) {
            switch($this->getCurrentToken()->getType()) {
                ...
                case Twig_Token::BLOCK_START_TYPE:
                    ...
                    if(null === $subparser) {
                        if(null !== $test) {
                            throw new Twig_Error_Syntax(sprintf('Unexpected tag name "%s" (expecting closing tag for the "%s" tag defined near line %s)',
                                                                $token->getValue(), $test[0]->getTag(),
                                                                $lineno), $token->getLine(), $this->stream->getFilename());
                        }
